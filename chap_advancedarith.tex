...
\end{lstlisting}
	...
	First off, we are initializing two variables, \texttt{x} and \texttt{y}. 
...
\end{lstlisting}
 	...
 	Before the power of 3 is applied to \texttt{x}+1, 1 needs to be added to \texttt{x}. 
...
\end{lstlisting}	
	...
	The \texttt{pow} operation is simple to use, and can make operations used in a program simpler from both a computing and visual stand point.

\LevelE{\texttt{sqrt()}}
	Square root operations are called in the program with \texttt{sqrt}. 
